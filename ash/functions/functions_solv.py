...
from ash.interfaces.interface_ORCA import checkORCAfinished, finalenergiesgrab, create_orca_inputVIEcomp_pc, \
    create_orca_inputVIEcomp_gas, create_orca_inputVIE_pc
from ash.modules.module_coords import get_molecule_members_loop, distance
...
# Define System Class. Useful for storing information about the system
class SystemAB:
    def __init__(self, name, chargeA, multA, chargeB, multB, solutetypesA, solutetypesB, solventtypes, snapslist,
                 snapshotsA, snapshotsB, solvtype):
        self.snapdir = 'snaps'
        self.Name = name
        self.ChargeA = chargeA
        self.MultA = multA
...
def create_AB_inputfiles_ORCA(solute_atoms, solvent_atoms, solvsphere, snapshots, orcasimpleinput,
    ...
    for fragfile in snapshots:
        ...
        if len(mmatoms) + len(qmatoms) != len(coords):
            print("MM atoms: {} and QM atoms: {} . Sum: {} . Differs from total number {}".format(len(mmatoms),
                                                                                                  len(qmatoms),
                                                                                                  len(mmatoms) + len(
                                                                                                      qmatoms),
                                                                                                  len(coords)))
        ...
        if bulkcorr is True:
            ...
            create_orca_inputVIEcomp_pc(name + identifier, inpname_AB, qmregion_elems, qmregion_coords, orcasimpleinput,
                                        orcablockinput,
                                        solvsphere.ChargeA, solvsphere.MultA, solvsphere.ChargeB, solvsphere.MultB,
                                        solvsphere.soluteatomsA, '')
        elif 'Gas' in identifier:
            create_orca_inputVIEcomp_gas(name + identifier, inpname_AB, qmregion_elems, qmregion_coords,
                                         orcasimpleinput, orcablockinput,
                                         solvsphere.ChargeA, solvsphere.MultA, solvsphere.ChargeB, solvsphere.MultB)
        else:
            ...
            create_orca_inputVIEcomp_pc(name + identifier, inpname_AB, qmregion_elems, qmregion_coords, orcasimpleinput,
                                        orcablockinput,
                                        solvsphere.ChargeA, solvsphere.MultA, solvsphere.ChargeB, solvsphere.MultB,
                                        solvsphere.soluteatomsA, solvbasis)
...
# OLD: TO BE DELETED
def create_AB_inputfiles_old(solvsphere, snapshotsA, snapshotsB, orcasimpleinput, orcablockinput, solventunitcharges,
                             bulkcorr=False):
    snaphotinpfiles = []
    for X in ['A', 'B']:
        if X == 'A':
        ...
        for fragfile in snapshots:
            ...
            create_orca_inputVIE_pc(name, inpname_AB, solute_elems, solute_coords, orcasimpleinput, orcablockinput,
                                    solvsphere.ChargeA, solvsphere.MultA, solvsphere.ChargeB, solvsphere.MultB)
            snaphotinpfiles.append(inpname_AB + '.inp')
    return snaphotinpfiles
...
def print_redox_output_state(state, solvsphere, orca_LL, orca_HL, snapshots, ave_traj, stdev_traj, repsnap_ave_traj,
    ...
    print("LRPol-Region1 LL-{} for {} repsnapshots: {} ± {}".format(Etype, len(repsnaplist), LRPol_ave_traj_R1,
                                                                    LRPol_stdev_traj_R1))
    print("LRPol-Region2 LL-{} for {} repsnapshots: {} ± {}".format(Etype, len(repsnaplist), LRPol_ave_traj_R2,
                                                                    LRPol_stdev_traj_R2))
