...
def read_cube(cubefile):
    ...
    for line in a:
        ...
        if count > 6 and count <= 6 + numatoms:
            ...
            coord_ang = [bohrang * float(line.split()[2]), bohrang * float(line.split()[3]),
                         bohrang * float(line.split()[4])]
            coords.append(coord)
    ...
    finaldict = {'rlowx': rlowx, 'dx': dx, 'nx': nx, 'orgx': orgx, 'rlowy': rlowy, 'dy': dy, 'ny': ny, 'orgy': orgy,
                 'rlowz': rlowz, 'dz': dz, 'nz': nz, 'orgz': orgz, 'elems': elems,
...
def write_cube(cubedict, name="Default"):
    with open(name + ".cube", 'w') as file:
        ...
        file.write(
            "{:>5}   {:9.6f}   {:9.6f}   {:9.6f}\n".format(cubedict['numatoms'], cubedict['orgx'], cubedict['orgy'],
                                                           cubedict['orgz']))
        ...
        for v1, v2 in zip(val1, val2):
            ...
            if len(v1) == 6:
                file.write("   {:.6e}   {:.6e}   {:.6e}   {:.6e}   {:.6e}   {:.6e}\n".format(cubeprod[0], cubeprod[1],
                                                                                             cubeprod[2], cubeprod[3],
                                                                                             cubeprod[4], cubeprod[5]))
            elif len(v1) == 5:
                file.write(
                    "   {:.6e}   {:.6e}   {:.6e}   {:.6e}   {:.6e}\n".format(cubeprod[0], cubeprod[1], cubeprod[2],
                                                                             cubeprod[3], cubeprod[4]))
            elif len(v1) == 4:
                file.write(
...
def DDEC_calc(elems=None, theory=None, gbwfile=None, numcores=1, DDECmodel='DDEC3', calcdir='DDEC',
    ...
    if chargemolbinarydir is None:
        ...
        print(
            "Please provide path as argument to DDEC_calc or put the location inside the $PATH variable on your Unix/Linux OS.")
        ashexit()
    ...
    spindictionary = {'H': 2, 'He': 1, 'Li': 2, 'Be': 1, 'B': 2, 'C': 3, 'N': 4, 'O': 3, 'F': 2, 'Ne': 1, 'Na': 2,
                      'Mg': 1, 'Al': 2, 'Si': 3, 'P': 4, 'S': 3, 'Cl': 2, 'Ar': 1, 'K': 2, 'Ca': 1, 'Sc': 2, 'Ti': 3,
                      'V': 4, 'Cr': 7, 'Mn': 6, 'Fe': 5, 'Co': 4, 'Ni': 3, 'Cu': 2, 'Zn': 1, 'Ga': 2, 'Ge': 3, 'As': 4,
                      'Se': 3, 'Br': 2, 'Kr': 1, 'Rb': 2, 'Sr': 1, 'Y': 2, 'Zr': 3, 'Nb': 6, 'Mo': 7, 'Tc': 6, 'Ru': 5,
                      'Rh': 4, 'Pd': 1, 'Ag': 2, 'Cd': 1, 'In': 2, 'Sn': 3, 'Sb': 4, 'Te': 3, 'I': 2, 'Xe': 1, 'Cs': 2,
                      'Ba': 1, 'La': 2, 'Ce': 1, 'Pr': 4, 'Nd': 5, 'Pm': 6, 'Sm': 7, 'Eu': 8, 'Gd': 9, 'Tb': 6, 'Dy': 5,
                      'Ho': 4, 'Er': 3, 'Tm': 2, 'Yb': 1, 'Lu': 2, 'Hf': 3, 'Ta': 4, 'W': 5, 'Re': 6, 'Os': 5, 'Ir': 4,
                      'Pt': 3, 'Au': 2, 'Hg': 1, 'Tl': 2, 'Pb': 3, 'Bi': 4, 'Po': 3, 'At': 2, 'Rn': 1, 'Fr': 2, 'Ra': 1,
                      'Ac': 2, 'Th': 3, 'Pa': 4, 'U': 5, 'Np': 6, 'Pu': 7, 'Am': 8, 'Cm': 9, 'Bk': 6, 'Cf': 5, 'Es': 5,
                      'Fm': 3, 'Md': 2, 'No': 1, 'Lr': 2, 'Rf': 3, 'Db': 4, 'Sg': 5, 'Bh': 6, 'Hs': 5, 'Mt': 4, 'Ds': 3,
                      'Rg': 2, 'Cn': 1, 'Nh': 2, 'Fl': 3, 'Mc': 4, 'Lv': 3, 'Ts': 2, 'Og': 1}
...
def DDEC_to_LJparameters(elems, molmoms, voldict, scale_polarH=False):
    ...
    for count, el in enumerate(elems):
        ...
        C6inkcal = ash.constants.harkcal * (
                    ash.dictionaries_lists.elems_C6_polz[atmnumber].C6 ** (1 / 6) * ash.constants.bohr2ang) ** 6
...
def select_indices_from_occupations(occlist, selection_thresholds=[1.98, 0.02]):
    ...
    indices_for_thresholds = [i for i, j in enumerate(occlist) if
                              j < selection_thresholds[0] and j > selection_thresholds[1]]
...
def difference_density_ORCA(fragment_A=None, fragment_B=None, theory_A=None, theory_B=None, griddensity=80,
                            cubefilename='difference_density'):
...
def NOCV_density_ORCA(fragment_AB=None, fragment_A=None, fragment_B=None, theory=None, griddensity=80,
    ...
    print("-" * 120)
    print(
        "Performing ORCA noiter calculation in order to orthogonalize orbitals and get file: promolecule_AB_orthog.gbw")
    print("-" * 120)
...
"""
    ...
    if make_cube_files is True:
        ...
        print("-" * 120)
        print(
            "Reading Cubefiles and creating difference density (i.e. deformation density) from orthogonalized promolecular density and final density")
        print("-" * 120)
    ...
    if make_cube_files is True:
        ...
        print("-" * 120)
        print(
            "Creating final Cube files for NOCV pair orbitals, orbital-densities and orbital-pair deformation densities")
        print("-" * 120)
        try:
        ...
        for i in range(0, num_nocv_pairs):
            ...
            os.rename(f"calcAB.nocv.mo{num_mos - 1 - i}a.cube",
                      f"calcAB.NOCVpair_{i}.acceptor_mo{num_mos - 1 - i}a.cube")
            print("Creating density for orbital")
            create_density_from_orb(f"calcAB.NOCVpair_{i}.acceptor_mo{num_mos - 1 - i}a.cube", denswrite=True,
                                    LargePrint=False)
            ...
            os.rename(f"calcAB.NOCVpair_{i}.donor_mo{i}a.cube",
                      f"NOCV_orbitals_and_densities/calcAB.NOCVpair_{i}.donor_mo{i}a.cube")
            os.rename(f"calcAB.NOCVpair_{i}.acceptor_mo{num_mos - 1 - i}a.cube",
                      f"NOCV_orbitals_and_densities/calcAB.NOCVpair_{i}.acceptor_mo{num_mos - 1 - i}a.cube")
            os.rename(f"calcAB.NOCVpair_{i}.donor_mo{i}a-dens.cube",
                      f"NOCV_orbitals_and_densities/calcAB.NOCVpair_{i}.donor_mo{i}a-dens.cube")
            os.rename(f"calcAB.NOCVpair_{i}.acceptor_mo{num_mos - 1 - i}a-dens.cube",
                      f"NOCV_orbitals_and_densities/calcAB.NOCVpair_{i}.acceptor_mo{num_mos - 1 - i}a-dens.cube")
...
def NOCV_Multiwfn(fragment_AB=None, fragment_A=None, fragment_B=None, theory=None, gridlevel=2, openshell=False,
    ...
    if fockmatrix_approximation == "initial" or fockmatrix_approximation == "final":
        ...
        print(
            "Warning: dE(orb) term is approximated when calculated by Multiwfn (as the correct TS Fock matrix is not used)")
    print("dE(int) is the vertical total interaction energy (without geometric relaxation)")
...
def make_molden_file(fragment, AO_basis, MO_coeffs, MO_energies=None, MO_occs=None, AO_order=None, label="ASH_orbs",
                     spherical_MOs=True):
...
def ASH_write_integralfile(two_el_integrals=None, one_el_integrals=None, nuc_repulsion_energy=None,
                           header_format="MRCC",
...
ScalHFX {percentages[1]}
    ...
    theory_1b = ORCATheory(orcasimpleinput=f"! {functional} {basis} CALCGUESSENERGY noiter ", orcablocks=blocks_1,
                           filename="theory1b",
