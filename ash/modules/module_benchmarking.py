            ...
            if '#' not in line:
                ...
                if corrections is True:
                    newreaction = BenchReaction(index, filenames, stoichiometry, refenergy, unit,
                                                correction=corrections_dict[index])
                else:
                    newreaction = BenchReaction(index, filenames, stoichiometry, refenergy, unit)
...
def run_benchmark(set=None, theory=None, numcores=None, reuseorbs=False, corrections=None, keepoutputfiles=True):
    ...
    if property != 'energy':
        ...
        for systemindex in database_dict:
            ...
            # Creating fragment
            frag = ash.Fragment(xyzfile=benchmarksetpath + system.filename + '.xyz', readchargemult=True,
                                conncalc=False)
    ...
    # REACTION ENERGY
        ...
        for reactionindex in database_dict:
            ...
            reaction_energy, error = ash.ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_energies=energies,
                                                        unit=unit, label=reactionindex,
                                                        reference=reaction.refenergy)
    ...
    # Print nice table
    print(BC.WARNING,
          "{:7s} {:55s}  {:13s} {:13s} {:13s}   {:17s}".format("Index", Proptype, "Ref.", "Calc.", "Calc.+corr.",
                                                               "Error"), BC.END)
    print("-" * 120)
    for rindex in database_dict:
        ...
        if property == 'energy':
            ...
            print(
                " {:<7} {:<80s}  {:<13.4f} {:<13.4f} {:<13.4f}{} {:>8.4f}{}".format(rindex, reactionstring, r.refenergy,
                                                                                    r.calcenergy,
                                                                                    r.calcenergy_corrected, colorcode,
                                                                                    r.error, BC.END))
        else:
            # print(" {:<10} {:<40s}  {:<13.4f} {:<13.4f}{} {:<13.4f}{}".format(rindex, ' '.join(r.filenames), r.refenergy, r.calcenergy, colorcode, r.error,BC.END))
            print(
                " {:<7} {:<80s}  {:<13.4f} {:<13.4f} {:<13.4f}{} {:>8.4f}{}".format(rindex, str(r.filename), r.refvalue,
                                                                                    r.calcvalue, r.calcvalue_corrected,
                                                                                    colorcode, r.error, BC.END))
