...
def create_ML_training_data(xyz_dir=None, dcd_trajectory=None, xyz_trajectory=None, num_snapshots=None,
    ...
    with open("train_data_mace.xyz", "w") as mace_file:
        ...
        for el, an_at in energies_atoms_dict.items():
            ...
            if Grad:
                mace_file.write(
                    f"Properties=species:S:1:pos:R:3:forces_REF:R:3 config_type=IsolatedAtom energy_REF={en_ev} pbc='F F F'\n")
                mace_file.write(f"{el:2s}{0.0:17.8f}{0.0:17.8f}{0.0:17.8f}"
                                f"{-0.0:17.8f}{-0.0:17.8f}{-0.0:17.8f}\n")
            else:
                mace_file.write(
                    f"Properties=species:S:1:pos:R:3 config_type=IsolatedAtom energy_REF={en_ev} pbc='F F F'\n")
                mace_file.write(f"{el:2s}{0.0:17.8f}{0.0:17.8f}{0.0:17.8f}\n")
        ...
        for i in range(len(energies)):
            ...
            if Grad:
                ...
                mace_file.write(
                    f"Properties=species:S:1:pos:R:3:molID:I:1:forces_REF:R:3 Nmols={Nmols} Comp={comp} energy_REF={energy_ev} pbc='F F F'\n")
                for j in range(frag.numatoms):  # Bug fix: inner loop variable was i, now j
                    mace_file.write(
                        f"{frag.elems[j]:2s}{frag.coords[j][0]:17.8f}{frag.coords[j][1]:17.8f}{frag.coords[j][2]:17.8f}"
                        f"{molindex:9d}{force[j][0]:17.8f}{force[j][1]:17.8f}{force[j][2]:17.8f}\n")
            else:
                # Write without forces
                mace_file.write(
                    f"Properties=species:S:1:pos:R:3:molID:I:1 Nmols={Nmols} Comp={comp} energy_REF={energy_ev} pbc='F F F'\n")
                for j in range(frag.numatoms):  # Bug fix: inner loop variable was i, now j
                    mace_file.write(
                        f"{frag.elems[j]:2s}{frag.coords[j][0]:17.8f}{frag.coords[j][1]:17.8f}{frag.coords[j][2]:17.8f}"
                        f"{molindex:9d}\n")
    ...
    if Grad:
        ...
        for file, stats in dbdict.items():
            try:
                ...
                print(
                    f"{file:<20s} {grads['rmse'] * gradscaling:8.6f} {grads['mae'] * gradscaling:8.6f} {grads['corr_coef']:8.4f} \
...
def query_by_committee(mltheories=None, configs=None, Grad=True, charge=0, mult=1, selection='energy-stdev',
                       threshold=0.1, num_snaps=5, label=""):
    print("-" * 50)
    ...
    try:
        ...
        eplot.addseries(0, x_list=list(range(0, len(stdevs_e))), y_list=stdevs_e, label='Stdev-E', color='blue',
                        line=True, scatter=True)
        eplot.savefig(f"stdevE_per_snap_{label}")

        # Create ASH_plot object named edplot
        eplot = ASH_plot("Plotname", num_subplots=1, x_axislabel="x-axis", y_axislabel='y-axis')
        eplot.addseries(0, x_list=list(range(0, len(stdevs_g))), y_list=stdevs_g, label='Stdev-G', color='blue',
                        line=True, scatter=True)
        eplot.savefig(f"stdevG_per_snap_{label}")

        # Create ASH_plot object named edplot
        eplot = ASH_plot("Plotname", num_subplots=1, x_axislabel="x-axis", y_axislabel='y-axis')
        eplot.addseries(0, x_list=list(range(0, len(stdevs_g_p))), y_list=stdevs_g_p, label='Stdev-Gp', color='blue',
                        line=True, scatter=True)
...
def active_learning(ml_theories=None, e_f_weights=None, training_dir=None, maxiter=10, theory_1=None, theory_2=None,
                    Grad=True,
