...
from ash.functions.functions_general import ashexit, listdiff, clean_number, blankline, BC, print_time_rel, \
...
from ash.modules.module_coords import check_charge_mult, read_xyzfile, write_multi_xyz_file, \
    Fragment
...
def NumFreq(fragment=None, theory=None, charge=None, mult=None, npoint=2, displacement=0.005, hessatoms=None,
    ...
    if hessatoms is None:
        ...
        if isinstance(theory, QMMMTheory):
            ...
            print(
                "Please provide a list of atom indices to the hessatoms keyword of NumFreq to define the partial Hessian")
            print(
                "For ONIOM numerical frequencies you want the list of hessatoms to be the same atoms used to define the \nactive-region in the optimization (or Region1)")
            print("Exiting now.")
            ashexit()
        else:
            hessatoms = allatoms
    ...
    ash.interfaces.interface_ORCA.write_ORCA_Hessfile(hessian, hesscoords, hesselems, hessmasses, hessatoms,
                                                      "orcahessfile.hess")
...
def printfreqs(vfreq, numatoms, TRmodenum=6, intensities=None, Raman_activities=None):
    ...
    if intensities is None:
        print(
            "No IR intensities were calculated (dipoles not available in QM-program interface). Setting values to 0.0.")
...
def get_center(coords, masses=None, elems=None, printlevel=2):
    if masses is None:
        ...
        masses = [ash.modules.module_coords.atommasses[ash.modules.module_coords.elematomnumbers[el.lower()] - 1] for el
                  in elems]
...
""".format(model)
    orcadummycalc = ash.interfaces.interface_ORCA.ORCATheory(orcasimpleinput=orcasimple, orcablocks=orcablocks,
                                                             extraline=extraline)
...
def approximate_full_Hessian_from_smaller(fragment, hessian_small, small_atomindices, large_atomindices=None,
    ...
    # large_atomindices not provided
        ...
        usedfragment = ash.Fragment(elems=subelems, coords=subcoords, printlevel=0, charge=fragment.charge,
                                    mult=fragment.mult)
    else:
        print("small_atomindices:", small_atomindices)
        print("large_atomindices:", large_atomindices)
