...
from ash.functions.functions_general import print_line_with_mainheader, ashexit
...
def load_matplotlib(backend='Agg'):
    ...
    try:
        ...
        print(
            "Loading MatplotLib failed. Probably not installed. Please install using conda: conda install matplotlib or pip: pip install matplotlib")
        return None
        # ashexit()
...
class ASH_plot():
    def __init__(self, figuretitle='Plottyplot', num_subplots=1, dpi=200, imageformat='png', figsize=(9, 5),
                 x_axislabel='X-axis', y_axislabel='Energy (X)', x_axislabels=None, y_axislabels=None,
                 title='Plot-title',
    ...
    def addseries(self, subplot, surfacedictionary=None, x_list=None, y_list=None, x_labels=None, label='Series',
                  color='blue', pointsize=40,
                  scatter=True, line=True, bar=False, scatter_linewidth=2, line_linewidth=1, barwidth=None,
                  barcolors=None, marker='o', legend=True, x_scaling=1.0, y_scaling=1.0,
...
def reactionprofile_plot(surfacedictionary, finalunit='', label='Label', x_axislabel='Coord', y_axislabel='Energy',
                         dpi=200, mode='pyplot',
                         imageformat='png', RelativeEnergy=True, pointsize=40, scatter_linewidth=2, line_linewidth=1,
                         color='blue',
                         filename='Plot'):
    ...
    conversionfactor = {'a.u.': 1.0, 'Eh': 1.0, 'au': 1.0, 'kcal/mol': 627.50946900, 'kcal/mol': 627.50946900,
                        'kJ/mol': 2625.499638, 'kJpermol': 2625.499638,
...
def contourplot(surfacedictionary, label='Label', x_axislabel='Coord', y_axislabel='Coord', finalunit='kcal/mol',
                interpolation='Cubic',
    ...
    conversionfactor = {'kcal/mol': 627.50946900, 'kcal/mol': 627.50946900, 'kJ/mol': 2625.499638,
...
def plot_Spectrum(xvalues=None, yvalues=None, plotname='Spectrum', range=None, unit='eV', broadening=0.1, points=10000,
                  imageformat='png', dpi=200, matplotlib=True, CSV=True, invert_xaxis=False, color='blue',
                  plot_sticks=True, lineshape='Gaussian', voigt_broadening=None):
    ...
    if lineshape == 'Gaussian':
        ...
        if voigt_broadening is None:
            print(
                "voigt_broadening (list of sigma and gamma values) not provided. Using broadening value for both sigma and gamma")
            sigma = broadening
            gamma = broadening
...
def MOplot_vertical(mos_dict, pointsize=4000, linewidth=2, label="Label", yrange=[-30, 3], imageformat='png'):
    ...
    if Openshell == True:
        ...
        ax.scatter([1.05] * len(virtbands_beta), virtbands_beta, color='pink', marker='_', s=pointsize,
                   linewidth=linewidth)
