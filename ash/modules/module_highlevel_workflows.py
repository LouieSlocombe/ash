...
from ash.interfaces.interface_ORCA import CCSD_natocc_grab, MP2_natocc_grab, CASSCF_natocc_grab, \
    UHF_natocc_grab, ORCA_orbital_setup
...
basisfamilies = ['cc', 'aug-cc', 'cc-dkh', 'cc-dk', 'aug-cc-dkh', 'aug-cc-dk', 'def2', 'ma-def2', 'def2-zora',
                 'def2-dkh', 'def2-dk',
                 'ma-def2-zora', 'ma-def2-dkh', 'ma-def2-dk', 'cc-CV', 'aug-cc-CV', 'cc-CV-dkh', 'cc-CV-dk',
                 'aug-cc-CV-dkh', 'aug-cc-CV-dk',
...
class ORCA_CC_CBS_Theory:
    def __init__(self, elements=None, scfsetting='TightSCF', extrainputkeyword='', extrablocks='', guessmode='Cmatrix',
                 memory=5000, numcores=1,
                 cardinals=None, basisfamily=None, Triplesextrapolation=False, SCFextrapolation=True, alpha=None,
                 beta=None,
                 ...
                 FullLMP2Guess=False, MP2_PNO_correction=False,
                 OOCC=False, T1=False, T1correction=False, T1corrbasis_size='Small',
                 T1corrpnosetting='NormalPNOreduced',
        ...
        if elements == None:
            print(BC.FAIL, "\nORCA_CC_CBS_Theory requires a list of elements to be given in order to set up basis sets",
                  BC.END)
            print(
                "Example: ORCA_CC_CBS_Theory(elements=['C','Fe','S','H','Mo'], basisfamily='def2',cardinals=[2,3], ...")
        ...
        if F12 == False and basisfamily == "cc-f12":
            ...
            print(
                "To use F12 instead of extrapolation set: F12=True, basisfamily='cc-f12', cardinals=[X] (i.e. single cardinal)",
                BC.END)
            ashexit()
        if F12 is True and 'f12' not in basisfamily:
            print(BC.FAIL, "F12 option chosen but an F12-basisfamily was not chosen. Choose basisfamily='cc-f12'",
                  BC.END)
            ashexit()
        ...
        if len(cardinals) == 1:
            print(BC.WARNING, "Only a single cardinal was chosen. This means that no extrapolation will be carried out",
                  BC.END)
...
{}\n""".format(memory, str(stabilityanalysis).lower(), guessmode, dlpno_line, extrablocks)
        ...
        if self.relativity == None:
            if self.basisfamily in ['cc-dkh', 'aug-cc-dkh', 'cc-dk', 'aug-cc-dk', 'def2-zora', 'def2-dkh', 'def2-dk',
                                    'cc-CV_3dTM-cc_L', 'aug-cc-CV_3dTM-cc_L'
                                                       'ma-def2-zora', 'ma-def2-dkh', 'cc-CV-dk', 'cc-CV-dkh',
                                    'aug-cc-CV-dk', 'aug-cc-CV-dkh']:
                print("Relativity option is None but a relativistic basis set family chosen:", self.basisfamily)
                print(
                    "You probably want relativity keyword argument set to DKH or ZORA (relativity=\"NoRel\" option possible also but not recommended)")
                ashexit()
        ...
        if self.DFTreference != None:
            if self.DFT_RI is True:
                self.extrainputkeyword = self.extrainputkeyword + ' {} RIJCOSX SARC/J defgrid3 '.format(
                    self.DFTreference)
            else:
        ...
        self.ccsd_line = "! {} {} {} {} {} {} printbasis".format(self.ccsdkeyword, self.mainbasiskeyword,
                                                                 self.pnokeyword, self.scfsetting,
                                                                 self.extrainputkeyword, self.auxbasiskeyword)
        self.ccsdt_line = "! {} {} {} {} {} {} printbasis".format(self.ccsdtkeyword, self.mainbasiskeyword,
                                                                  self.pnokeyword, self.scfsetting,
                                                                  self.extrainputkeyword, self.auxbasiskeyword)
        ...
        if self.singlebasis is True:
            ...
            self.ccsdt_1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                    orcasimpleinput=self.ccsdt_line,
            ...
            if self.Triplesextrapolation == True:
                ...
                self.ccsd_1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                       orcasimpleinput=self.ccsd_line,
                                                                       orcablocks=self.blocks1, numcores=self.numcores)
                self.ccsd_2 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                       orcasimpleinput=self.ccsd_line,
                                                                       orcablocks=self.blocks2, numcores=self.numcores)
                # CCSD(T) for first and second cardinals
                self.ccsdt_0 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                        orcasimpleinput=self.ccsdt_line,
                                                                        orcablocks=self.blocks0, numcores=self.numcores)
                self.ccsdt_1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                        orcasimpleinput=self.ccsdt_line,
                                                                        orcablocks=self.blocks1, numcores=self.numcores)
            else:
                ...
                self.ccsdt_1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                        orcasimpleinput=self.ccsdt_line,
                                                                        orcablocks=self.blocks1, numcores=self.numcores)
                self.ccsdt_2 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                        orcasimpleinput=self.ccsdt_line,
                                                                        orcablocks=self.blocks2, numcores=self.numcores)
    ...
    def MP2correction_Step(self, current_coords, elems, calc_label, numcores, charge=None, mult=None, basis=None,
                           pnosetting='NormalPNO', PC=None, current_MM_coords=None, MMcharges=None):
...
""" + blocks
    ...
    TCutMKN {thresholdsetting["TCutMKN"]}
        ...
        can_mp2 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir, orcasimpleinput=can_mp2_line,
                                                           orcablocks=blocks, numcores=self.numcores)
        mp2_energy = can_mp2.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge,
                                 mult=mult, PC=PC, current_MM_coords=current_MM_coords, MMcharges=MMcharges)
        ...
        dlpno_mp2 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir, orcasimpleinput=dlpno_mp2_line,
                                                             orcablocks=blocks + '\n' + mp2block,
                                                             numcores=self.numcores)
        dlpno_mp2_energy = dlpno_mp2.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge,
                                         mult=mult, PC=PC, current_MM_coords=current_MM_coords, MMcharges=MMcharges)
    ...
    TCutMKN {thresholdsetting["TCutMKN"]}
        ...
        # Defining theory
        ccsdt_T1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir, orcasimpleinput=ccsdt_T1_line,
                                                            orcablocks=blocks, numcores=self.numcores)
        ...
        triples_T1 = float(
            pygrep('Triples Correction (T)                     ...', ccsdt_T1.filename + '.out')[-1].split()[-1])
    ...
    def CVSR_Step(self, current_coords, elems, reloption, calc_label, numcores, charge=None, mult=None, PC=None,
                  current_MM_coords=None, MMcharges=None):
        init_time = time.time()
        print("\nCVSR_Step")

        ccsdt_mtsmall_NoFC_line = "! {} {} {}   nofrozencore {} {} {}".format(self.ccsdtkeyword, self.CVbasis,
                                                                              self.auxbasiskeyword, self.pnokeyword,
                                                                              self.scfsetting, self.extrainputkeyword)
        ccsdt_mtsmall_FC_line = "! {} {}  {} {} {} {}".format(self.ccsdtkeyword, self.CVbasis, self.auxbasiskeyword,
                                                              self.pnokeyword, self.scfsetting, self.extrainputkeyword)

        ccsdt_mtsmall_NoFC = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                      orcasimpleinput=ccsdt_mtsmall_NoFC_line,
                                                                      orcablocks=self.blocks, numcores=self.numcores)
        ccsdt_mtsmall_FC = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                    orcasimpleinput=ccsdt_mtsmall_FC_line,
                                                                    orcablocks=self.blocks, numcores=self.numcores)

        # Run
        energy_ccsdt_mtsmall_nofc = ccsdt_mtsmall_NoFC.run(elems=elems, current_coords=current_coords,
                                                           numcores=numcores, charge=charge, mult=mult, PC=PC,
                                                           current_MM_coords=current_MM_coords, MMcharges=MMcharges)
    ...
    def PNOExtrapolationStep(self, elems=None, current_coords=None, theory=None, calc_label=None, numcores=None,
                             charge=None, mult=None, triples=True, PC=None, current_MM_coords=None, MMcharges=None):
...
TCutMKN {thresholdsetting["TCutMKN"]}
        ...
        theory.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge, mult=mult, PC=PC,
                   current_MM_coords=current_MM_coords, MMcharges=MMcharges)
        PNOcalcX_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(theory.filename + '.out',
                                                                                DLPNO=self.DLPNO, F12=self.F12)
        ...
        theory.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge, mult=mult, PC=PC,
                   current_MM_coords=current_MM_coords, MMcharges=MMcharges)
        PNOcalcY_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(theory.filename + '.out',
                                                                                DLPNO=self.DLPNO, F12=self.F12)
    ...
    def run(self, current_coords=None, qm_elems=None,
            elems=None, Grad=False, numcores=None, charge=None, mult=None, PC=None, current_MM_coords=None,
            MMcharges=None, mm_elems=None):
        ...
        for element in elems:
            if element not in self.Calc1_basis_dict:
                ...
                print(
                    "Make sure to pass a list of all elements of molecule/benchmark-database when creating ORCA_CC_CBS_Theory object")
        ...
        if numelectrons == 1:
            ...
            # Components
            E_dict = {'Total_E': E_total, 'E_CC_CBS': E_total, 'E_SCF_CBS': E_total, 'E_corrCCSD_CBS': 0.0,
                      'E_corrCCT_CBS': 0.0, 'T1energycorr': 0.0,
                      'E_corr_CBS': 0.0, 'E_SO': 0.0, 'E_corecorr_and_SR': 0.0, 'E_HOCC': 0.0, 'E_DBOC': 0.0,
                      'E_FCIcorrection': 0.0}
        ...
        if self.pnosetting == "extrapolation":
            ...
            print(
                "Will run 2 jobs with PNO thresholds TCutPNO : {:e} and {:e} for each basis set cardinal calculation".format(
                    self.pnoextrapolation[0], self.pnoextrapolation[1]))
            ...
            if self.singlebasis is True:
                ...
                E_SCF_CBS, E_corrCCSD_CBS, E_corrCCT_CBS, E_corr_CBS = self.PNOExtrapolationStep(elems=elems,
                                                                                                 current_coords=current_coords,
                                                                                                 theory=self.ccsdt_1,
                                                                                                 calc_label=calc_label + 'cardinal1',
                                                                                                 numcores=numcores,
                                                                                                 charge=charge,
                                                                                                 mult=mult, PC=PC,
            ...
            # EXTRAPOLATION WITH 2 BASIS SETS
                ...
                if self.Triplesextrapolation == True:
                    ...
                    print("=" * 70)
                    print("Now doing Basis-2 CCSD job: Family: {} Cardinal: {} ".format(self.basisfamily,
                                                                                        self.cardinals[2]))
                    print("=" * 70)
                    E_SCF_2, E_corrCCSD_2, E_corrCCT_2, E_corrCC_2 = self.PNOExtrapolationStep(elems=elems,
                                                                                               current_coords=current_coords,
                                                                                               theory=self.ccsd_2,
                                                                                               calc_label=calc_label + 'cardinal2',
                                                                                               numcores=numcores,
                                                                                               charge=charge, mult=mult,
                                                                                               triples=False, PC=PC,
                                                                                               current_MM_coords=current_MM_coords,
                                                                                               MMcharges=MMcharges)  # No triples

                    # Medium-basis CCSDT(T) job
                    print("Next running CCSD(T) calculations to get separate triples contribution:")

                    print("=" * 70)
                    print("Now doing Basis-1 CCSD(T) job: Family: {} Cardinal: {} ".format(self.basisfamily,
                                                                                           self.cardinals[1]))
                    print("=" * 70)

                    E_SCF_1, E_corrCCSD_1, E_corrCCT_1, E_corrCC_1 = self.PNOExtrapolationStep(elems=elems,
                                                                                               current_coords=current_coords,
                                                                                               theory=self.ccsdt_1,
                                                                                               calc_label=calc_label + 'cardinal1',
                                                                                               numcores=numcores,
                    ...
                    print("=" * 70)
                    print("Now doing Basis-0 CCSD(T) job: Family: {} Cardinal: {} ".format(self.basisfamily,
                                                                                           self.cardinals[0]))
                    print("=" * 70)

                    E_SCF_0, E_corrCCSD_0, E_corrCCT_0, E_corrCC_0 = self.PNOExtrapolationStep(elems=elems,
                                                                                               current_coords=current_coords,
                                                                                               theory=self.ccsdt_0,
                                                                                               calc_label=calc_label + 'cardinal0',
                                                                                               numcores=numcores,
                                                                                               charge=charge, mult=mult,
                                                                                               triples=True, PC=PC,
                                                                                               current_MM_coords=current_MM_coords,
                    ...
                    print(
                        f"triplescorr_energies (cardinals:{self.cardinals[0]},{self.cardinals[1]}) :{triplescorr_energies}")

                    # BASIS SET EXTRAPOLATION
                    # SCF extrapolation. WIll be overridden inside function if self.SCFextrapolation==True
                    print("\nSCF extrapolation:")
                    E_SCF_CBS = Extrapolation_twopoint_SCF(scf_energies, [self.cardinals[1], self.cardinals[2]],
                                                           self.basisfamily,
                                                           alpha=self.alpha,
                                                           SCFextrapolation=self.SCFextrapolation)  # 2-point SCF extrapolation

                    # Separate CCSD, (T) and full-corr CBS energies
                    print("\nCCSD corr. extrapolation:")
                    E_corrCCSD_CBS = Extrapolation_twopoint_corr(ccsdcorr_energies,
                                                                 [self.cardinals[1], self.cardinals[2]],
                                                                 self.basisfamily,
                                                                 beta=self.beta)  # 2-point extrapolation using smaller cardinals
                    print("\n(T) corr. extrapolation:")
                    E_corrCCT_CBS = Extrapolation_twopoint_corr(triplescorr_energies,
                                                                [self.cardinals[0], self.cardinals[1]],
                                                                self.basisfamily,
                    ...
                    E_SCF_1, E_corrCCSD_1, E_corrCCT_1, E_corrCC_1 = self.PNOExtrapolationStep(elems=elems,
                                                                                               ...
                                                                                               MMcharges=MMcharges)
                    print("=" * 70)
                    print("Basis-1 job done. Now doing Basis-2 job: Family: {} Cardinal: {} ".format(self.basisfamily,
                                                                                                     self.cardinals[1]))
                    print("=" * 70)
                    E_SCF_2, E_corrCCSD_2, E_corrCCT_2, E_corrCC_2 = self.PNOExtrapolationStep(elems=elems,
                                                                                               current_coords=current_coords,
        ...
        # OR no PNO extrapolation
            ...
            if self.singlebasis is True:
                self.ccsdt_1.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge,
                                 mult=mult, PC=PC, current_MM_coords=current_MM_coords, MMcharges=MMcharges)
                CCSDT_1_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(self.ccsdt_1.filename + '.out',
                                                                                       DLPNO=self.DLPNO, F12=self.F12)
                ...
                if self.MP2_PNO_correction is True:
                    CCSDT_1_MP2_CPS_corr = self.MP2correction_Step(current_coords, elems, "CCSDT_1step", numcores,
                                                                   charge=charge, mult=mult, basis=1,
                                                                   pnosetting=self.pnosetting, PC=PC,
                                                                   current_MM_coords=current_MM_coords,
                                                                   MMcharges=MMcharges)
                else:
            ...
            # REGULAR CBS EXTRAPOLATION WITHOUT PNO EXTRAPOLATION:
                ...
                if self.Triplesextrapolation == True:
                    ...
                    self.ccsd_2.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge,
                                    mult=mult, PC=PC, current_MM_coords=current_MM_coords, MMcharges=MMcharges)
                    CCSD_2_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(self.ccsd_2.filename + '.out',
                                                                                          DLPNO=self.DLPNO)
                    ...
                    if self.MP2_PNO_correction is True:
                        CCSDT_2_MP2_CPS_corr = self.MP2correction_Step(current_coords, elems, "CCSDT_2step", numcores,
                                                                       charge=charge, mult=mult, basis=2,
                                                                       pnosetting=self.pnosetting, PC=PC,
                                                                       current_MM_coords=current_MM_coords,
                                                                       MMcharges=MMcharges)
                    else:
                    ...
                    self.ccsdt_0.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge,
                                     mult=mult, PC=PC, current_MM_coords=current_MM_coords, MMcharges=MMcharges)
                    CCSDT_0_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(
                        self.ccsdt_0.filename + '.out', DLPNO=self.DLPNO)
                    ...
                    CCSDT_1_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(
                        self.ccsdt_1.filename + '.out', DLPNO=self.DLPNO)
                    shutil.copyfile(self.ccsdt_1.filename + '.out', './' + calc_label + 'CCSDT_1' + '.out')
                    shutil.copyfile(self.ccsdt_1.filename + '.gbw', './' + calc_label + 'CCSDT_1' + '.gbw')
                    print("CCSDT_1_dict:", CCSDT_1_dict)
                    ...
                    ccsdcorr_energies = [CCSDT_1_dict['CCSD_corr'] + CCSDT_1_MP2_CPS_corr,
                                         CCSD_2_dict['CCSD_corr'] + CCSDT_2_MP2_CPS_corr]
                ...
                # REGULAR DIRECT CCSD(T) EXTRAPOLATION
                    ...
                    self.ccsdt_2.run(elems=elems, current_coords=current_coords, numcores=numcores, charge=charge,
                                     mult=mult, PC=PC, current_MM_coords=current_MM_coords, MMcharges=MMcharges)
                    CCSDT_2_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(
                        self.ccsdt_2.filename + '.out', DLPNO=self.DLPNO)
                    ...
                    ccsdcorr_energies = [CCSDT_1_dict['CCSD_corr'] + CCSDT_1_MP2corr,
                                         CCSDT_2_dict['CCSD_corr'] + CCSDT_2_MP2corr]
        ...
        if self.T1 == False:
            if self.T1correction == True:
                T1energycorr = self.T1correction_Step(current_coords, elems, calc_label, numcores, charge=charge,
                                                      mult=mult,
        ...
        if self.CVSR is True:
            ...
            if self.ECPflag is True:
                print(
                    "ECPs present. Not doing ScalarRelativistic Correction. Switching to Core-Valence Correction only.")
                reloption = " "
                ...
                E_corecorr_and_SR = self.CVSR_Step(current_coords, elems, reloption, calc_label, numcores,
                                                   charge=charge, mult=mult, PC=PC, current_MM_coords=current_MM_coords,
                                                   MMcharges=MMcharges)
            else:
                reloption = "DKH"
        ...
        if self.DBOCcorrection is True:
            ...
            if self.DBOC_program == 'CFour':
                ...
                E_DBOC = run_CFour_DBOC_correction(coords=current_coords, elems=elems, charge=charge, mult=mult,
                                                   method=self.DBOC_method, basis=self.DBOC_basis,
                                                   numcores=original_numcores,
                                                   openshell=openshell)
        else:
            E_DBOC = 0.0
        ############################################################
        ...
        # Components
        E_dict = {'Total_E': E_FINAL, 'E_CC_CBS': E_CC_CBS, 'E_SCF_CBS': E_SCF_CBS, 'E_corrCCSD_CBS': E_corrCCSD_CBS,
                  'E_corrCCT_CBS': E_corrCCT_CBS, 'T1energycorr': T1energycorr,
                  'E_corr_CBS': E_corr_CBS, 'E_SO': E_SO, 'E_corecorr_and_SR': E_corecorr_and_SR, 'E_HOCC': E_HOCC,
                  'E_DBOC': E_DBOC,
                  'E_FCIcorrection': E_FCIcorrection}
...
class MRCC_CC_CBS_Theory:
    def __init__(self, elements=None, scfsetting='TightSCF', extrainputkeyword='', extrablocks='', memory=5000,
                 numcores=1,
        ...
        # CHECKS to exit early
        if elements == None:
            print(BC.FAIL, "\nMRCC_CC_CBS_Theory requires a list of elements to be given in order to set up basis sets",
                  BC.END)
            print(
                "Example: MRCC_CC_CBS_Theory(elements=['C','Fe','S','H','Mo'], basisfamily='def2',cardinals=[2,3], ...")
            print("Should be a list containing all elements that a fragment might contain")
            ashexit()
        else:
            # Removing redundant symbols (in case fragment.elems list was passed for example)
    ...
    def run(self, current_coords=None, elems=None, Grad=False, numcores=None, charge=None, mult=None, PC=None,
            current_MM_coords=None, MMcharges=None, mm_elems=None):
        ...
        for element in elems:
            if element not in self.Calc1_basis_dict:
                print("Error. No basis-set definition available for element: {}".format(element))
                print(
                    "Make sure to pass a list of all elements of molecule/benchmark-database when creating CC_CBS_Theory object")
        ...
        if numelectrons == 1:
            ...
            print("Using hardcoded value: ", E_total)
            if self.FCI is True:
                E_dict = {'Total_E': E_total, 'E_SCF_CBS': E_total, 'E_CC_CBS': E_total, 'E_FCI_CBS': E_total,
                          'E_corrCCSD_CBS': 0.0,
        ...
        # REGULAR EXTRAPOLATION WITH 2 THEORIES
            ...
            CCSDT_2_dict = ash.interfaces.interface_ORCA.grab_HF_and_corr_energies(self.ccsdt_2.filename + '.out',
                                                                                   DLPNO=self.DLPNO)
            shutil.copyfile(self.ccsdt_2.filename + '.out', './' + calc_label + 'CCSDT_2' + '.out')
            shutil.copyfile(self.ccsdt_2.filename + '.gbw', './' + calc_label + 'CCSDT_2' + '.gbw')
            print("CCSDT_2_dict:", CCSDT_2_dict)
        ...
        if self.FCI is True:
            ...
            E_FCIcorrection = E_FCI_CBS - E_CC_CBS
            E_FINAL = E_FCI_CBS
            E_dict = {'Total_E': E_FINAL, 'E_FCI_CBS': E_FCI_CBS, 'E_CC_CBS': E_CC_CBS, 'E_SCF_CBS': E_SCF_CBS,
                      'E_corrCCSD_CBS': E_corrCCSD_CBS,
                      'E_corrCCT_CBS': E_corrCCT_CBS, 'E_corr_CBS': E_corr_CBS, 'E_SO': E_SO,
                      'E_corecorr_and_SR': E_corecorr_and_SR, 'E_FCIcorrection': E_FCIcorrection}
            print("FCI correction:", E_FCIcorrection, "Eh")
            ...
            print("CCSD(T)/CBS energy :", E_CC_CBS, "Eh")
            E_FINAL = E_CC_CBS
            E_dict = {'Total_E': E_FINAL, 'E_CC_CBS': E_CC_CBS, 'E_SCF_CBS': E_SCF_CBS,
                      'E_corrCCSD_CBS': E_corrCCSD_CBS, 'E_corrCCT_CBS': E_corrCCT_CBS,
...
extrapolation_parameters_dict = {'cc_23': [4.42, 2.460], 'aug-cc_23': [4.30, 2.510], 'cc_34': [5.46, 3.050],
                                 'aug-cc_34': [5.790, 3.050],
                                 'def2_23': [10.390, 2.4], 'def2_34': [7.880, 2.970], 'pc_23': [7.02, 2.01],
                                 'pc_34': [9.78, 4.09], 'ma-def2_23': [10.390, 2.4],
...
def Extrapolation_twopoint_corr(corr_energies, cardinals, basis_family, beta=None):
    ...
    corr_final = (math.pow(cardinals[0], beta) * corr_energies[0] - math.pow(cardinals[1], beta) * corr_energies[1]) / (
            math.pow(cardinals[0], beta) - math.pow(cardinals[1], beta))
...
def basis_for_element(element, basisfamily, cardinal):
    ...
    if basisfamily == "cc":
        ...
        if cardinal == 6 and element not in ['H', 'He', 'Be', 'B', 'C', 'N', 'O', 'F', 'Ne', 'Al', 'Si', 'P', 'S', 'Cl',
                                             'Ar']:
    ...
    # ALL-ELECTRON CORRELATION-CONSISTENT BASIS SETS FOR DKH calculations
        ...
        if atomnumber <= 18:
            ...
            print(BC.FAIL,
                  "cc-pV5Z-DK basis sets for elements beyond Kr is missing in ORCA. Take a look at literature.", BC.END)
            ashexit()
        ...
        if atomnumber <= 18:
            ...
            print(BC.FAIL,
                  "aug-cc-pV5Z-DK basis sets for elements beyond Kr is missing in ORCA. Take a look at literature.",
                  BC.END)
            ashexit()
    ...
    # Core-valence cc-basis sets (cc-pCVnZ or cc-pwCVnZ) for DKH
        ...
        if atomnumber <= 4:
            ...
            print("Warning. No proper CV basis set available for element {}. Using valence basis set instead".format(
                element))
    ...
    print(BC.FAIL,
          "There is probably no {} {}Z basis set available for element {} in ORCA. Exiting.".format(basisfamily,
                                                                                                    cardinal, element),
          BC.END)
    ashexit()
...
"""
    icetheory = ash.ORCATheory(orcasimpleinput=input, orcablocks=blocks, numcores=numcores,
                               basis_per_element=basis_per_element, label=f'{label}ICE_tgen{tgen}_tvar_{tvar}',
                               save_output_with_label=True)
    return icetheory


# New Workflow to do ICE-CI calculation using CASSCF module using any input orbitals
# no orbital selection
def newAuto_ICE_CAS(fragment=None, basis="cc-pVDZ", basisblock="", moreadfile=None, nmin=1.98, nmax=0.02, extrainput="",
                    nat_occupations=None,
                    gtol=2.50e-04, numcores=1, charge=None, mult=None, CASCI=True, tgen=1e-4, memory=10000,
                    no_moreadfile_in_CAS=False,
                    ciblockline=""):
    if fragment is None:
    ...
    if CASCI is True:
        ...
        print(
            "In order to get ICE-CI natural orbitals we don't use the noiter option but instead change the CASSCF convergence threshold so that only 1 iteration is carried out")
    ...
    nel, norb = ash.functions.functions_elstructure.select_space_from_occupations(nat_occupations,
                                                                                  selection_thresholds=[nmin, nmax])
    ...
    ice_cas_CI = ash.ORCATheory(orcasimpleinput=f"! CASSCF {basis} tightscf",
                                orcablocks=casblocks, moreadfile=moreadfile, autostart=autostart, label=f"{label}",
                                numcores=numcores)
...
def Auto_ICE_CAS(fragment=None, basis="cc-pVDZ", nmin=1.98, nmax=0.02, extrainput="", nroots=1,
                 initial_orbitals="RI-MP2", MP2_density="unrelaxed", CC_density="unrelaxed", moreadfile=None,
                 gtol=2.50e-04,
    ...
    if initial_orbitals == "HF" or initial_orbitals == "RHF":
        ...
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} HF {basis}  tightscf", numcores=numcores,
                                 label='RHF', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        ...
        exit()
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} UHF {basis} normalprint UNO tightscf",
                                 numcores=numcores,
                                 label='UHF', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        ...
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} UHF {basis}  UNO tightscf", numcores=numcores,
                                 label='UHF-QRO', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.qro"
        natoccgrab = UHF_natocc_grab
    elif initial_orbitals == "MP2":
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} MP2 {basis} autoaux tightscf", orcablocks=mp2blocks,
                                 numcores=numcores,
                                 label='MP2', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mp2nat"
        natoccgrab = MP2_natocc_grab
    elif initial_orbitals == "RI-MP2":
        natorbs = ash.ORCATheory(orcasimpleinput=f"! RI-MP2 {basis} autoaux tightscf", orcablocks=mp2blocks,
                                 numcores=numcores,
                                 label='MP2', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mp2nat"
        ...
        natorbs = ash.ORCATheory(orcasimpleinput=f"! CCSD {basis} autoaux tightscf", orcablocks=ccsdblocks,
                                 numcores=numcores,
                                 label='CCSD', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mdci.nat"
        natoccgrab = CCSD_natocc_grab
    elif initial_orbitals == "DLPNO-CCSD":
        natorbs = ash.ORCATheory(orcasimpleinput=f"! DLPNO-CCSD {basis} autoaux tightscf", orcablocks=ccsdblocks,
                                 numcores=numcores,
                                 label='DLPNOCCSD', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mdci.nat"
        natoccgrab = CCSD_natocc_grab
    else:
    ...
    ice_cas_CI = ash.ORCATheory(orcasimpleinput=f"! CASSCF  {basis} tightscf", filename="ice_cas_ci",
                                orcablocks=casblocks, moreadfile=mofile, autostart=autostart, label=f"{label}",
                                numcores=numcores)
...
def Reaction_FCI_Analysis(reaction=None, basis=None, basisfile=None, basis_per_element=None,
                          Do_ICE_CI=True,
                          Do_TGen_fixed_series=True, fixed_tvar=1e-11, Do_Tau3_series=True, Do_Tau7_series=True,
                          Do_EP_series=True,
                          tgen_thresholds=None, ice_nmin=1.999, ice_nmax=0,
                          separate_MP2_nat_initial_orbitals=True,
                          DoHF=True, DoMP2=True, DoCC=True, DoCC_CCSD=True, DoCC_CCSDT=True, DoCC_MRCC=False,
                          DoCC_Bruckner=False, DoCC_CFour=False, DoCAS=False,
    ...
    if separate_MP2_nat_initial_orbitals is True:
        ...
        """
        natmp2 = ash.ORCATheory(orcasimpleinput=f"! MP2 {basis} autoaux tightscf", orcablocks=mp2blocks,
                                basis_per_element=basis_per_element, numcores=numcores, label='MP2',
                                save_output_with_label=True)
        for frag in reaction.fragments:
            ...
            nel, norb = ash.functions.functions_elstructure.select_space_from_occupations(step1occupations,
                                                                                          selection_thresholds=[
                                                                                              upper_sel_threshold,
                                                                                              lower_sel_threshold])
            print(
                f"Selecting CAS({nel},{norb}) based on thresholds: upper_sel_threshold={upper_sel_threshold} and lower_sel_threshold={lower_sel_threshold}")
        ...
        largest_fragindex = [i[0] for i in reaction.properties["CAS"]].index(
            max([i[0] for i in reaction.properties["CAS"]]))
        ...
        if Do_EP_series is True and Do_Tau3_series is True:
                ...
                for tgen in tgen_thresholds:
                    if Do_Tau3_series:
                        ...
                        if len(E_tot_tau3) > 1:
                            ...
                            twop_slope, twop_E_extrap = np.polyfit([abs(j) for j in E_pt2_tau3[-2:]], E_tot_tau3[-2:],
                                                                   1)
                            # 3-point extrap. with last 3 points
                            threep_slope, threep_E_extrap = np.polyfit([abs(j) for j in E_pt2_tau3[-3:]],
                                                                       E_tot_tau3[-3:], 1)
            ...
            if Do_Tau3_series:
                ...
                for i, tgen in enumerate(E_extrap_tau3_2p[0]):
                    ...
                    reaction_energy_2p = \
                        ash.ReactionEnergy(list_of_energies=list_of_energies_2p, stoichiometry=reaction.stoichiometry,
                ...
                for i, tgen in enumerate(E_extrap_tau3_3p[0]):
                    ...
                    reaction_energy_3p = \
                        ash.ReactionEnergy(list_of_energies=list_of_energies_3p, stoichiometry=reaction.stoichiometry,
                                           unit=reaction.unit, silent=False)[0]
    ...
    if DoHF is True:
        ...
        %maxcore 11000
        """
        hf = ash.ORCATheory(orcasimpleinput=f"! HF {basis} tightscf", orcablocks=hfblocks,
                            basis_per_element=basis_per_element, numcores=numcores, label='HF',
                            save_output_with_label=True)
    ...
    if DoMP2 is True:
        ...
        %maxcore 11000
        """
        mp2 = ash.ORCATheory(orcasimpleinput=f"! MP2 {basis} tightscf", orcablocks=mp2blocks,
                             basis_per_element=basis_per_element, numcores=numcores, label='MP2',
                             save_output_with_label=True)
        scsmp2 = ash.ORCATheory(orcasimpleinput=f"! SCS-MP2 {basis} tightscf", orcablocks=mp2blocks,
                                basis_per_element=basis_per_element, numcores=numcores, label='SCSMP2',
                                save_output_with_label=True)
        ...
        if Do_OOMP2 is True:
            oomp2 = ash.ORCATheory(orcasimpleinput=f"! OO-RI-MP2 autoaux {basis} tightscf", orcablocks=mp2blocks,
                                   basis_per_element=basis_per_element, numcores=numcores, label='OOMP2',
                                   save_output_with_label=True)
            scsoomp2 = ash.ORCATheory(orcasimpleinput=f"! OO-RI-SCS-MP2 {basis} autoaux tightscf", orcablocks=mp2blocks,
                                      basis_per_element=basis_per_element, numcores=numcores, label='OOSCSMP2',
                                      save_output_with_label=True)
    ...
    if DoCC is True:
        ...
        newnumcores = check_cores_vs_electrons(reaction.fragments[smallest_frag].elems, numcores,
                                               reaction.fragments[smallest_frag].charge)
        ...
        if DoCC_CCSD is True:
            ccsd = ash.ORCATheory(orcasimpleinput=f"! CCSD {basis} tightscf", orcablocks=ccblocks,
                                  basis_per_element=basis_per_element, numcores=numcores, label='CCSD',
                                  save_output_with_label=True)
            bccd = ash.ORCATheory(orcasimpleinput=f"! CCSD {basis} tightscf", orcablocks=brucknerblocks,
                                  basis_per_element=basis_per_element, numcores=numcores, label='BCCD',
                                  save_output_with_label=True)
            ...
            if Do_OOCC is True:
                ooccd = ash.ORCATheory(orcasimpleinput=f"! OOCCD {basis} tightscf", orcablocks=ccblocks,
                                       basis_per_element=basis_per_element, numcores=numcores, label='OOCCD',
                                       save_output_with_label=True)
        ...
        if DoCC_CCSDT is True:
            ccsdt = ash.ORCATheory(orcasimpleinput=f"! CCSD(T) {basis} tightscf", orcablocks=ccblocks,
                                   basis_per_element=basis_per_element, numcores=numcores, label='CCSDT',
                                   save_output_with_label=True)
            ccsdt_qro = ash.ORCATheory(orcasimpleinput=f"! UHF CCSD(T) {basis} UNO tightscf", orcablocks=ccblocks,
                                       basis_per_element=basis_per_element, numcores=numcores, label='CCSDT_QRO',
                                       save_output_with_label=True)
            # CCSD(T) extrapolated to FCI
            if basis_per_element == None:
                ccsdt_fci_extrap = ORCA_CC_CBS_Theory(elements=reaction.fragments[0].elems, cardinals=[2],
                                                      basisfamily="cc", numcores=1, FCI=True)
                ...
                print(
                    "Warning: Basisdict per element provided. Can currently not do CCSD(T) FCI ORCA_CC_CBS_Theory job. SKipping")
            ...
            ccsdt_qro.cleanup()
            if DoCC_Bruckner is True:
                bccdt = ash.ORCATheory(orcasimpleinput=f"! CCSD(T) {basis} tightscf", orcablocks=brucknerblocks,
                                       basis_per_element=basis_per_element, numcores=numcores, label='BCCDT',
                                       save_output_with_label=True)
            ...
            if Do_OOCC is True:
                ooccdt = ash.ORCATheory(orcasimpleinput=f"! OOCCD(T) {basis} tightscf", orcablocks=ccblocks,
                                        basis_per_element=basis_per_element, numcores=numcores, label='OOCCDT',
                                        save_output_with_label=True)
            ...
            if DoCC_DFTorbs is True:
                ...
                for functional in KS_functionals:
                    ...
                    ccsdt_dft = ash.ORCATheory(orcasimpleinput=f"! CCSD(T) {functional} {basis} autoaux tightscf",
                                               orcablocks=ccblocks, numcores=numcores, label=f'CCSDT_{functional}',
                                               save_output_with_label=True)
    ...
    """
            ...
            casscf = ash.ORCATheory(orcasimpleinput=f"! CASSCF {basis} tightscf", orcablocks=casblocks,
                                    basis_per_element=basis_per_element,
                                    numcores=numcores, label=caslabel, save_output_with_label=True,
                                    moreadfile=reaction.orbital_dictionary["MP2nat"][i])
            caspt2 = ash.ORCATheory(orcasimpleinput=f"! CASPT2 {basis} tightscf", orcablocks=casblocks,
                                    basis_per_element=basis_per_element,
                                    numcores=numcores, label=caslabel, save_output_with_label=True)
            nevpt2 = ash.ORCATheory(orcasimpleinput=f"! NEVPT2 {basis} tightscf", orcablocks=casblocks,
                                    basis_per_element=basis_per_element,
                                    numcores=numcores, label=caslabel, save_output_with_label=True)
            mrci_q = ash.ORCATheory(orcasimpleinput=f"! MRCI+Q {basis} tightscf", orcablocks=casblocks,
                                    basis_per_element=basis_per_element,
                                    numcores=numcores, label=caslabel, save_output_with_label=True)
        ...
        # Reaction energies
        reaction_energy_casscf = \
            ash.ReactionEnergy(list_of_energies=casscf_energies, stoichiometry=reaction.stoichiometry,
                               unit=reaction.unit, silent=False)[0]
        reaction_energy_caspt2 = \
            ash.ReactionEnergy(list_of_energies=caspt2_energies, stoichiometry=reaction.stoichiometry,
                               unit=reaction.unit, silent=False)[0]
        reaction_energy_nevpt2 = \
            ash.ReactionEnergy(list_of_energies=nevpt2_energies, stoichiometry=reaction.stoichiometry,
                               unit=reaction.unit, silent=False)[0]
        reaction_energy_mrciq = \
            ash.ReactionEnergy(list_of_energies=mrciq_energies, stoichiometry=reaction.stoichiometry,
                               unit=reaction.unit, silent=False)[0]
    ...
    if plot is True:
        ...
        if DoCAS is True:
            eplot = ASH_plot("Plotname", num_subplots=3, x_axislabels=["TGen", "Method", "Method"],
                             y_axislabels=[f'{y_axis_label} ({reaction.unit})', f'', f''],
                             subplot_titles=[f"ICE-CI/{basislabel}", f"Singleref./{basislabel}",
                                             f"Multiref./{basislabel}"],
                             ylimit=ylimits, horizontal=True, padding=padding)
        else:
            eplot = ASH_plot("Plotname", num_subplots=2, x_axislabels=["TGen", "Method"],
                             y_axislabels=[f'{y_axis_label} ({reaction.unit})', f'{y_axis_label} ({reaction.unit})', ],
                             subplot_titles=[f"ICE-CI/{basislabel}", f"Singleref./{basislabel}"],
                             ylimit=ylimits, horizontal=True, padding=padding)
        print("eplot:", eplot)
        if eplot.working is not False:
            ...
            if Do_ICE_CI is True:
                ...
                if Do_Tau3_series:
                    eplot.addseries(0, x_list=tgen_thresholds, y_list=tau3_e_series, label="Tau3", color='blue',
                                    line=True, scatter=True, x_scale_log=True)
                if Do_Tau7_series:
                    eplot.addseries(0, x_list=tgen_thresholds, y_list=tau7_e_series, label="Tau7", color='orange',
                                    line=True, scatter=True, x_scale_log=True)
                if Do_TGen_fixed_series:
                    eplot.addseries(0, x_list=tgen_thresholds, y_list=taufixed_e_series, label=f"Tvar:{fixed_tvar}",
                                    color='green', line=True, scatter=True, x_scale_log=True)
                if Do_EP_series:
                    if Do_Tau3_series:
                        eplot.addseries(0, x_list=tgen_thresholds[1:], y_list=tau3_EP_series_2p,
                                        label=f"EP-Tau3-2point", color='black', line=True, scatter=True,
                                        x_scale_log=True)
                        eplot.addseries(0, x_list=tgen_thresholds[2:], y_list=tau3_EP_series_3p,
                                        label=f"EP-Tau3-3point", color='purple', line=True, scatter=True,
                                        x_scale_log=True)
            # Plotting method labels on x-axis with rotation to make things fit
            eplot.addseries(1, x_list=SR_WF_indices, y_list=SR_WF_energies, x_labels=SR_WF_labels, label=reaction.label,
                            color='red', line=True, scatter=True, xticklabelrotation=80)
            # Plotting method labels on x-axis with rotation to make things fit
            if DoCAS is True:
                eplot.addseries(2, x_list=MR_WF_indices, y_list=MR_WF_energies, x_labels=MR_WF_labels,
                                label=reaction.label, color='purple', line=True, scatter=True, xticklabelrotation=80)
            # Save figure
...
def Reaction_FCI_correction(reaction=None, basis=None, basis_per_element=None, numcores=1, maxcorememory=4000,
    ...
    for i, frag in enumerate(reaction.fragments):
        ...
        """
        # ORCA-CCSD(T)
        ccsdt_orca = ash.ORCATheory(orcasimpleinput=ccline, orcablocks=ccblocks, basis_per_element=basis_per_element,
                                    numcores=numcores, label='CCSDT', save_output_with_label=True)
        ...
        """
        ccsdt_mrcc = ash.MRCCTheory(mrccinput=ccsdt_mrccinput, numcores=numcores, label='CCSDT',
                                    save_output_with_label=True)
    ...
    reaction_energy_ice = \
        ash.ReactionEnergy(list_of_energies=ice_energies, stoichiometry=reaction.stoichiometry, unit=reaction.unit,
                           silent=False)[0]
    reaction_energy_cc_orca = \
        ash.ReactionEnergy(list_of_energies=ccsdt_energies_orca, stoichiometry=reaction.stoichiometry,
                           unit=reaction.unit,
                           silent=False)[0]
    reaction_energy_cc_mrcc = \
        ash.ReactionEnergy(list_of_energies=ccsdt_energies_mrcc, stoichiometry=reaction.stoichiometry,
                           unit=reaction.unit,
                           silent=False)[0]
...
class ORCA_MRCI_CBS_Theory:
    def __init__(self, elements=None, scfsetting='TightSCF', extrainputkeyword='', extrablocks='', memory=5000,
                 numcores=1,
                 cardinals=None, basisfamily=None, SCFextrapolation=True, alpha=None, beta=None, orcadir=None,
                 relativity=None,
                 atomicSOcorrection=False, auxbasis="autoaux-max", MRPT2_method="CASPT2", MRHL_method="MRCI+Q",
                 MRHL_basis="small",
        ...
        if elements == None:
            print(BC.FAIL,
                  "\nORCA_MRCI_CBS_Theory requires a list of elements to be given in order to set up basis sets",
                  BC.END)
            print(
                "Example: ORCA_MRCI_CBS_Theory(elements=['C','Fe','S','H','Mo'], basisfamily='def2',cardinals=[2,3], ...")
            print("Should be a list containing all elements that a fragment might contain")
            ashexit()
        else:
            # Removing redundant symbols (in case fragment.elems list was passed for example)
...
{extrablocks}\n"""
        ...
        if self.singlebasis is True:
            # For single-basis CCSD(T) or single-basis F12 calculations
            self.mrpt2_1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                    orcasimpleinput=self.mrpt2_input_line,
                                                                    orcablocks=self.blocks1, numcores=self.numcores)
        else:
            # Extrapolations
            ...
            self.mrpt2_1 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                    orcasimpleinput=self.mrpt2_input_line,
                                                                    orcablocks=self.blocks1, numcores=self.numcores)
            self.mrpt2_2 = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                    orcasimpleinput=self.mrpt2_input_line,
                                                                    orcablocks=self.blocks2, numcores=self.numcores)
        # HL correction
        if self.MRHL_method != "None":
            if self.MRHL_basis == "small":
                self.mrhl = ash.interfaces.interface_ORCA.ORCATheory(orcadir=self.orcadir,
                                                                     orcasimpleinput=self.mrhl_input_line,
                                                                     orcablocks=self.blocks1, numcores=self.numcores)
...
diismaxeq 20
    ...
    MR_excitation_options_all = [
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 0, '2p': 0, '2h1p': 0, '1h2p': 0, '2h2p': 0, 'ss': 0, 'isss': 0, 'issa': 0,
         'ssss': 0, 'sssa': 0},
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 1, '2p': 1, '2h1p': 0, '1h2p': 0, '2h2p': 0, 'ss': 0, 'isss': 0, 'issa': 0,
         'ssss': 0, 'sssa': 0},
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 1, '2p': 1, '2h1p': 1, '1h2p': 0, '2h2p': 0, 'ss': 0, 'isss': 0, 'issa': 0,
         'ssss': 0, 'sssa': 0},
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 1, '2p': 1, '2h1p': 1, '1h2p': 1, '2h2p': 0, 'ss': 0, 'isss': 0, 'issa': 0,
         'ssss': 0, 'sssa': 0},
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 1, '2p': 1, '2h1p': 1, '1h2p': 1, '2h2p': 1, 'ss': 0, 'isss': 0, 'issa': 0,
         'ssss': 0, 'sssa': 0},
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 1, '2p': 1, '2h1p': 1, '1h2p': 1, '2h2p': 1, 'ss': 1, 'isss': 1, 'issa': 0,
         'ssss': 0, 'sssa': 0},
        {'1h': 1, '1p': 1, '1h1p': 1, '2h': 1, '2p': 1, '2h1p': 1, '1h2p': 1, '2h2p': 1, 'ss': 1, 'isss': 1, 'issa': 1,
         'ssss': 1, 'sssa': 1}]
...
"""
        mrtheory = ash.ORCATheory(orcasimpleinput=f"! {basis} CASSCF noiter", orcablocks=blocks, moreadfile=orbfile,
                                  numcores=numcores)
