...
def solvshell_v2(orcadir='', NumCores=None, calctype='', orcasimpleinput_LL='',
    ...
    if BulkCorrection == True:
        ...
        bulkinpfiles = ash.functions.functions_solv.create_AB_inputfiles_ORCA(solute_atoms, solvent_atoms, solvsphere,
                                                                              totrepsnaps,
                                                                              orcasimpleinput_LL, orcablockinput_LL,
                                                                              solventunitcharges, identifiername, None,
                                                                              bulkcorr)
        ...
        # GRAB output
        Bulk_Allrepsnaps_ABenergy, Bulk_Arepsnaps_ABenergy, Bulk_Brepsnaps_ABenergy = ash.functions.functions_solv.grab_energies_output_ORCA(
            bulkinpfiles)
        blankline()
    ...
    if ShortRangePolarization == True:
        ...
        SRPolinpfiles = ash.functions.functions_solv.create_AB_inputfiles_ORCA(solute_atoms, solvent_atoms, solvsphere,
                                                                               totrepsnaps,
                                                                               orcasimpleinput_SRPOL,
                                                                               orcablockinput_SRPOL, solventunitcharges,
                                                                               identifiername, SRPolShell, False,
                                                                               solvbasis)
        ...
        if SRORCAPar == True:
            ...
            print(
                "Using multiproc parallelization. All calculations running in parallel but each ORCA calculation using 1 core.")
            run_inputfiles_in_parallel(orcadir, SRPolinpfiles, NumCores)

        # GRAB output
        SRPol_Allrepsnaps_ABenergy, SRPol_Arepsnaps_ABenergy, SRPol_Brepsnaps_ABenergy = ash.functions.functions_solv.grab_energies_output_ORCA(
            SRPolinpfiles)
        blankline()
        ...
        if orcasimpleinput_SRPOL == orcasimpleinput_LL:
            ...
            SRPolinpfiles_Region1 = ash.functions.functions_solv.create_AB_inputfiles_ORCA(solute_atoms, solvent_atoms,
                                                                                           solvsphere, totrepsnaps,
                                                                                           orcasimpleinput_SRPOL,
                                                                                           orcablockinput_SRPOL,
                                                                                           solventunitcharges,
                                                                                           identifiername, None, False,
                                                                                           solvbasis)
            # Run the inputfiles
    ...
    if LongRangePolarization == True:
        ...
        if calctype == "redox":
            ...
            print("LRPol_Region2 calculation TrajB average: {:3.3f} ± {:3.3f}".format(LRPol_ave_trajB_Region2,
                                                                                      LRPol_stdev_trajB_Region2))
            ...
            print("LRPol_Region1 calculation TrajB average: {:3.3f} ± {:3.3f}".format(LRPol_ave_trajB_Region1,
                                                                                      LRPol_stdev_trajB_Region1))
            ...
            print("LRPolcorrections (A):",
                  list(np.array(LRPol_Arepsnaps_ABenergy_Region2) - np.array(LRPol_Arepsnaps_ABenergy_Region1)))
            print("LRPolcorrections (B):",
                  list(np.array(LRPol_Brepsnaps_ABenergy_Region2) - np.array(LRPol_Brepsnaps_ABenergy_Region1)))
            LRPolcorr_mean_A = statistics.mean(LRPolcorrdict_A.values())
            LRPolcorr_stdev_A = statistics.stdev(LRPolcorrdict_A.values())
            blankline()
            print("Traj A: LRPolcorrection {:3.3f} ± {:3.3f} eV".format(LRPolcorr_mean_A, LRPolcorr_stdev_A))
        ...
        LRPol_stdev_trajA = 0;
        LRPolcorr_mean_A = 0
        LRPol_ave_trajA_Region1 = 0;
        LRPol_stdev_trajA_Region1 = 0;
        LRPol_ave_trajA_Region2 = 0;
        LRPol_stdev_trajA_Region2 = 0
        LRPol_ave_trajB_Region1 = 0;
        LRPol_stdev_trajB_Region1 = 0;
        LRPol_ave_trajB_Region2 = 0;
        LRPol_stdev_trajB_Region2 = 0
        LRPol_ave_trajAB_Region1 = 0;
        LRPol_stdev_trajAB_Region1 = 0;
        LRPol_ave_trajAB_Region2 = 0;
        LRPol_stdev_trajAB_Region2 = 0
    blankline()
    ...
    if GasCorrection:
        ...
        gasinpfiles_LL = ash.functions.functions_solv.create_AB_inputfiles_ORCA(solute_atoms, [], solvsphere, gaslist,
                                                                                orcasimpleinput_LL,
                                                                                orcablockinput_LL, solventunitcharges,
                                                                                identifiername)
        identifiername = '_Gas_HL'
        gasinpfiles_HL = ash.functions.functions_solv.create_AB_inputfiles_ORCA(solute_atoms, [], solvsphere, gaslist,
                                                                                orcasimpleinput_HL,
                                                                                orcablockinput_HL, solventunitcharges,
                                                                                identifiername)
    ...
    if PrintFinalOutput == True:
        if calctype == "redox":
            ...
            ash.functions.functions_solv.print_redox_output_state("A", solvsphere, orcasimpleinput_LL,
                                                                  orcasimpleinput_HL, solvsphere.snapshotsA, ave_trajA,
                                                                  stdev_trajA,
                                                                  repsnap_ave_trajA, repsnap_stdev_trajA, repsnaplistA,
                                                                  Bulk_ave_trajA, Bulk_stdev_trajA, Bulkcorr_mean_A,
                                                                  SRPol_ave_trajA, SRPol_stdev_trajA, SRPolcorr_mean_A,
                                                                  LRPol_ave_trajA_Region1, LRPol_ave_trajA_Region2,
                                                                  LRPol_stdev_trajA_Region1, LRPol_stdev_trajA_Region2,
                                                                  LRPolcorr_mean_A, gasA_VIE_LL, gasA_VIE_HL)
            print_line_with_subheader1("Trajectory B")
            ash.functions.functions_solv.print_redox_output_state("B", solvsphere, orcasimpleinput_LL,
                                                                  orcasimpleinput_HL, solvsphere.snapshotsB, ave_trajB,
                                                                  stdev_trajB,
                                                                  repsnap_ave_trajB, repsnap_stdev_trajB, repsnaplistB,
                                                                  Bulk_ave_trajB, Bulk_stdev_trajB, Bulkcorr_mean_B,
                                                                  SRPol_ave_trajB, SRPol_stdev_trajB, SRPolcorr_mean_B,
                                                                  LRPol_ave_trajB_Region1, LRPol_ave_trajB_Region2,
                                                                  LRPol_stdev_trajB_Region1, LRPol_stdev_trajB_Region2,
                                                                  LRPolcorr_mean_B, gasB_VIE_LL, gasB_VIE_HL)
            print_line_with_subheader1("Final Average")
            ash.functions.functions_solv.print_redox_output_state("AB", solvsphere, orcasimpleinput_LL,
                                                                  orcasimpleinput_HL, solvsphere.snapshots, ave_trajAB,
                                                                  stdev_trajAB,
                                                                  repsnap_ave_trajAB, repsnap_stdev_trajAB,
                                                                  repsnaplistAB, Bulk_ave_trajAB, Bulk_stdev_trajAB,
                                                                  Bulkcorr_mean_AB,
                                                                  SRPol_ave_trajAB, SRPol_stdev_trajAB,
                                                                  SRPolcorr_mean_AB, LRPol_ave_trajAB_Region1,
                                                                  LRPol_ave_trajAB_Region2,
                                                                  LRPol_stdev_trajAB_Region1,
                                                                  LRPol_stdev_trajAB_Region2, LRPolcorr_mean_AB,
                                                                  gasAB_AIE_LL, gasAB_AIE_HL)
        else:
...
def Polsnapshotcalc(args):
    ...
    PEsolvshell = ash.functions.functions_solv.get_solvshell(solvsphere, snap_frag.elems, snap_frag.coords, LRPolRegion,
                                                             solute_elems,
                                                             solute_coords,
                                                             ash.settings_solvation.scale, ash.settings_solvation.tol)
    qm_solvshell = ash.functions.functions_solv.get_solvshell(solvsphere, snap_frag.elems, snap_frag.coords,
                                                              PolQMRegion, solute_elems,
                                                              solute_coords,
                                                              ash.settings_solvation.scale, ash.settings_solvation.tol)
    ...
    if len(qmatoms) + len(peatoms) + len(mmatoms) != len(solvsphere.allatoms):
        print(
            "QM + MM + PE atoms ({})not equal to total numatoms({})".format(len(qmatoms) + len(peatoms) + len(mmatoms),
