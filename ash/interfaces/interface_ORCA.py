...
from ash.functions.functions_general import ashexit, insert_line_into_file, BC, print_time_rel, \
...
{bstring}
        ...
        if self.xdm == True:
            dispE, dispgrad = ash.functions.functions_elstructure.xdm_run(wfxfile=self.filename + '.wfx',
...
def run_orca_SP(list):
    ...
    with open(basename + '.out', 'w') as ofile:
        process = sp.run([orcadir + '/orca', basename + '.inp'], check=True, stdout=ofile, stderr=ofile,
...
def MolecularOrbitalGrab(file):
    ...
    # Final dict
    MOdict = {"occ_alpha": bands_alpha, "occ_beta": bands_alpha, "unocc_alpha": virtbands_a, "unocc_beta": virtbands_b,
              "Openshell": Openshell,
    ...
    with open(hessfile) as hfile:
        for line in hfile:
            if cartgrab == True:
                count = count + 1
                elem = line.split()[0];
                x_c = ash.constants.bohr2ang * float(line.split()[2]);
                y_c = ash.constants.bohr2ang * float(line.split()[3]);
                z_c = ash.constants.bohr2ang * float(line.split()[4])
...
def write_ORCA_Hessfile(hessian, coords, elems, masses, hessatoms, outputname):
    ...
    for el, mass, coord in zip(elems, masses, coords):
        ...
        orcahessfile.write(" " + el + '    ' + str(mass) + "  " + str(coord[0] / ash.constants.bohr2ang) +
                           " " + str(coord[1] / ash.constants.bohr2ang) + " " + str(
            coord[2] / ash.constants.bohr2ang) + "\n")
    orcahessfile.write("\n")
    orcahessfile.write("\n")
    orcahessfile.close()
    print("")
...
def create_orca_inputVIEcomp_pc(name, name2, elems, coords, orcasimpleinput, orcablockinput, chargeA, multA, chargeB,
                                multB, soluteatoms, basisname):
    pcfile = name + '.pc'
...
def create_orca_inputVIE_pc(name, name2, elems, coords, orcasimpleinput, orcablockinput, chargeA, multA, chargeB,
                            multB):
    pcfile = name + '.pc'
    with open(name2 + '.inp', 'w') as orcafile:
        # Adding xyzfile to orcasimpleinput
...
def create_orca_inputVIEnewjob_gas(name, name2, elems, coords, orcasimpleinput, orcablockinput, chargeA, multA, chargeB,
                                   multB):
    with open(name2 + '.inp', 'w') as orcafile:
        # Adding xyzfile to orcasimpleinput
        orcasimpleinput = orcasimpleinput + ' xyzfile'
...
def counterpoise_calculation_ORCA(fragments=None, theory=None, monomer1_indices=None, monomer2_indices=None,
    ...
    if monomer1_indices == None or monomer2_indices == None:
        ...
        print(
            "Example: monomer1_indices=[0,1,2] (H2O monomer) and monomer2_indices=[3,4,5,6] (MeOH monomer) in an H2O...MeOH dimer with coordinates:")
...
H    2.453295744  -1.445998564  -1.389381355
    ...
    if len(monomer1_indices + monomer2_indices) != dimer.numatoms:
        print(
            "Error: Something wrong with monomer1_indices or monomer2_indices. Don't add up ({}) to number of atoms in dimer ({})".format(
                len(monomer1_indices + monomer2_indices), dimer.numatoms))
        ashexit()
    ...
    theory.cleanup()
    print("\nUncorrected binding energy: {} kcal/mol".format(
        (dimer_energy - monomer1_energy - monomer2_energy) * ash.constants.hartokcal))
    ...
    print("Sum of monomers at dimer geometry energy: {} Eh".format(
        monomer1_in_dimergeo_energy + monomer2_in_dimergeo_energy))

    # Monomers at dimer geometry and dimer basis (GHOSTATOMS)
    print("\nMonomer 1 at dimer geometry with dimer basis: {} Eh".format(
        monomer1_in_dimer_dimerbasis_energy))  # E_A^AB (AB)
    print("Monomer 2 at dimer geometry with dimer basis: {} Eh".format(
        monomer2_in_dimer_dimerbasis_energy))  # E_B^AB (AB)
    print("Sum of monomers at dimer geometry with dimer basis: {} Eh".format(
        monomer1_in_dimer_dimerbasis_energy + monomer2_in_dimer_dimerbasis_energy))

    #
    deltaE_unc = (dimer_energy - monomer1_energy - monomer2_energy) * ash.constants.hartokcal
    counterpoise_corr = -1 * (
            monomer1_in_dimer_dimerbasis_energy - monomer1_in_dimergeo_energy + monomer2_in_dimer_dimerbasis_energy - monomer2_in_dimergeo_energy) * ash.constants.hartokcal
...
def create_ASH_otool(basename=None, theoryfile=None, scriptlocation=None, charge=None, mult=None):
    ...
    with open(scriptlocation + "/otool_external", 'w') as otool:
        ...
        otool.write(
            "ash.interfaces.interface_ORCA.print_gradient_in_ORCAformat(energy,gradient,\"{}\")\n".format(basename))
...
def run_orca_mapspc(filename, option, start=0.0, end=100, unit='eV', broadening=1.0, points=5000, orcadir=None):
    ...
    p = sp.run([orcadir + '/orca_mapspc', filename, option, f"-{unit}" f"-w{broadening}", f"-n{points}"],
               encoding='ascii')
...
def orca_frag_guess(fragment=None, theory=None, A_indices=None, B_indices=None, A_charge=None, B_charge=None,
                    A_mult=None, B_mult=None):
    ...
    calc_AB = copy.copy(theory);
    calc_AB.filename = "calcAB"
    calc_A = copy.copy(theory);
    calc_A.filename = "calcA"
    calc_B = copy.copy(theory);
    calc_B.filename = "calcB"
...
def ORCA_orbital_setup(orbitals_option=None, fragment=None, basis=None, basisblock="", extrablock="", extrainput="",
                       label="frag",
                       MP2_density=None, MDCI_density=None, AutoCI_density=None, memory=10000, numcores=1, charge=None,
                       mult=None, moreadfile=None,
                       gtol=2.50e-04, nmin=1.98, nmax=0.02, CAS_nel=None, CAS_norb=None, CASCI=False,
                       natorb_iterations=None, AVAS=False,
    ...
    if basis is None:
        ...
        print(
            "Note: you can additionally used basisblock string to provide additional basis options or override keyword")
        ashexit()
    ...
    if 'MP2' in orbitals_option:
        print(
            "MP2-type orbitals requested. This means that natural orbitals will be created from the chosen MP2 density")
    ...
    if orbitals_option == 'CCSD':
        MDCIkeyword = "CCSD"
        print(
            "CCSD-type orbitals requested. This means that natural orbitals will be created from the chosen MDCI_density ")
    ...
    if orbitals_option == "DLPNO-CCSD":
        ...
        print(
            "DLPNO-CCSD-type orbitals requested. This means that natural orbitals will be created from the chosen MDCI_density ")
        if MDCI_density is None:
            print("Error: MDCI_density must be provided")
            print("Options: linearized, unrelaxed or orbopt")
            ashexit()
    ...
    if 'QCISD' in orbitals_option:
        MDCIkeyword = "QCISD"
        print(
            "QCISD-type orbitals requested. This means that natural orbitals will be created from the chosen MDCI_density")
        if MDCI_density is None:
            print("Error: MDCI_density must be provided")
            print("Options: linearized, unrelaxed or orbopt")
            ashexit()
    ...
    if 'CEPA/1' in orbitals_option:
        MDCIkeyword = "CEPA/1"
        print(
            "CEPA/1-type orbitals requested. This means that natural orbitals will be created from the chosen MDCI_density")
        if MDCI_density is None:
            print("Error: MDCI_density must be provided")
            print("Options: linearized, unrelaxed or orbopt")
            ashexit()
    ...
    if 'CPF/1' in orbitals_option:
        MDCIkeyword = "CPF/1"
        print(
            "CPF/1-type orbitals requested. This means that natural orbitals will be created from the chosen MDCI_density")
        if MDCI_density is None:
            print("Error: MDCI_density must be provided")
            print("Options: linearized, unrelaxed or orbopt")
            ashexit()
    ...
    if 'MR' in orbitals_option:
        ...
        print(
            "MR-type orbitals requested. This means that natural orbitals will be created from the chosen MRCI_density")
        if CAS_nel is None or CAS_nel is None:
    ...
    if 'FIC' in orbitals_option:
        ...
        print(
            "AUTOCI-type orbitals requested. This means that natural orbitals will be created from the chosen AUTOCI density")
        if CAS_nel is None or CAS_nel is None:
    ...
    if ROHF is True:
        ...
        print(
            "Warning: ORCA will attempt an ROHF calculation while guessing what type of open-shell case you are dealing with")
...
"""
        avas_prep = ash.ORCATheory(orcasimpleinput=f"!  {basis} {extrainput} noiter {AVAS_keyword}",
                                   orcablocks=avasblocks, numcores=numcores,
                                   label='AVASprep', filename="AVASprep", save_output_with_label=True,
                                   moreadfile=moreadfile, autostart=autostart_option)
...
"""
        mp2_prep = ash.ORCATheory(orcasimpleinput=f"! RI-MP2 {basis} {extrainput} autoaux tightscf",
                                  orcablocks=mp2blocks, numcores=numcores,
                                  label='MP2prep', filename="MP2prep", save_output_with_label=True,
                                  moreadfile=moreadfile, autostart=autostart_option)
    ...
    if orbitals_option == "HF" or orbitals_option == "RHF":
        ...
        if NMF_sigma is None:
            ...
            ashexit()
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} {basis}  tightscf", NMF=True, NMF_sigma=NMF_sigma,
                                 numcores=numcores,
                                 label='NMF', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.gbw"
        ...
        """
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} RI-MP2 {basis} autoaux tightscf",
                                 orcablocks=mp2blocks, numcores=numcores,
                                 label='RIMP2', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mp2nat"
        natoccgrab = MP2_natocc_grab
        ...
        """
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} RI-SCS-MP2 {basis} autoaux tightscf",
                                 orcablocks=mp2blocks, numcores=numcores,
                                 label='RI-SCS-MP2', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mp2nat"
        natoccgrab = MP2_natocc_grab
        ...
        """
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} OO-RI-MP2 {basis} autoaux tightscf",
                                 orcablocks=mp2blocks, numcores=numcores,
                                 label='OO-RI-MP2', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.mp2nat"
        natoccgrab = MP2_natocc_grab
        ...
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} {MDCIkeyword} {basis} autoaux tightscf",
                                 orcablocks=ccsdblocks, numcores=numcores,
                                 label=mdcilabel, save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        ...
        """
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} CASSCF {basis} tightscf", orcablocks=casscfblocks,
                                 numcores=numcores,
                                 label='CASSCF', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.gbw"
        ...
        """
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} {MRCIkeyword} {basis} autoaux tightscf",
                                 orcablocks=mrciblocks, numcores=numcores,
                                 label=MRCIkeyword, save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        ...
        """
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput} {AUTOCIkeyword} {basis} autoaux tightscf",
                                 orcablocks=autociblocks, numcores=numcores,
                                 label=AUTOCIkeyword, save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        natoccgrab = None
...
"""
        ...
        natorbs = ash.ORCATheory(orcasimpleinput=f"! {extrainput}  {basis} autoaux tightscf", orcablocks=mrciblocks,
                                 numcores=numcores,
                                 label='FOBO', save_output_with_label=True, autostart=autostart_option,
                                 moreadfile=moreadfile)
        mofile = f"{natorbs.filename}.b0_s0.nat"
        ...
        if natorb_iterations:
            ...
            # Special issues
                ...
                mofile, nat_occupations = new_ORCA_natorbsfile_from_density(natorbs.gbwfile, densityname="mdcip",
                                                                            result_file="ORCA_DLPNOCCSD_nat_ASH",
                                                                            ORCA_version="6.0.0",
                ...
                mofile, nat_occupations = new_ORCA_natorbsfile_from_density(natorbs.gbwfile, densityname="mdcip",
                                                                            result_file=f"ORCA_MDCI{orbitals_option}_nat_ASH",
                                                                            ORCA_version="6.0.0",
                                                                            change_from_UHF_to_ROHF=True)
                natocc_print(nat_occupations, orbitals_option, nmin, nmax)
                ...
                mofile, nat_occupations = new_ORCA_natorbsfile_from_density(natorbs.gbwfile, densityname="autocipur",
                                                                            result_file="ORCA_CCSD_T_nat_ASH",
                                                                            ORCA_version="6.0.0",
                                                                            change_from_UHF_to_ROHF=True)
                natocc_print(nat_occupations, orbitals_option, nmin, nmax)
            else:
    ...
    print("\nReturning name of orbital file that can be used in next ORCATheory calculation (moreadfile option):",
          newmofile)
...
def new_ORCA_natorbsfile_from_density(gbwfile, densityname="mdcip", result_file="ORCA_ASH", ORCA_version="6.0.0",
    ...
    if change_from_UHF_to_ROHF is True:
        ...
        mol_data["MolecularOrbitals"]["OrbitalLabels"] = mol_data["MolecularOrbitals"]["OrbitalLabels"][
            0:int(len(mol_data["MolecularOrbitals"]["OrbitalLabels"]) / 2)]
